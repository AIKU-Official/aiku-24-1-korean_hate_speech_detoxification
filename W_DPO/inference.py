...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset_path',type=int,default="/home/phw/work/KoMo/dataset/eval_dataset.csv")
    parser.add_argument('--save_folder',type=int,default="/home/phw/work/KoMo/inference_output/")
    ...
    data_30000 = [config.checkpoint_idx]#[1044,2088,1740,1392,2436,2784,3132,3480]

    for ch_idx in tqdm(data_30000,desc="current checkpoint "):
        print(f"current checkpoint : {ch_idx}\n")
        model_name = f"jeeyoung/dpo{ch_idx}0th_trial_40000_data"
        model_kwargs = {'device_map': 'balanced'} 
        policy_dtype = getattr(torch,"float32")
        #peft_config = PeftConfig.from_pretrained(model_name)
        #base_model_name = peft_config.base_model_name_or_path
        policy = transformers.AutoModelForCausalLM.from_pretrained(model_name, low_cpu_mem_usage=True, torch_dtype=policy_dtype, **model_kwargs)
        ...
        sample_list = pd.read_csv(data_path)['input_sentence'].to_list()
